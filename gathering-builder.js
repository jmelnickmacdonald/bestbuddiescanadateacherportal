...
(() => {
  ...
  const questions = [
    ...
    {
      key: "focus",
      title: "What would you like this gathering to do?",
      hint: "Choose the main goal for today. The plan can still do more than one thing.",
      type: "single",
      choices: [
        ["know", "Get to know each other"],
        ["friendship", "Build friendship & connection"],
    ...
    {
      key: "fit",
      title: "What would make this easier for your group?",
      hint: "Choose any that help. This is about removing barriers, not sorting people into categories. You can also skip this question.",
      type: "multi",
      optional: true,
      choices: [
        ["seated", "Seated or lower-movement options"],
        ["communication", "AAC, sign, gesture, pointing, or other ways to communicate"],
        ["reading", "Keep reading & writing light"],
        ["partner", "Partner or buddy support"],
        ["visual", "Visual or creative ways to take part"],
        ["lowpressure", "Easy to watch first, pass, or join without being put on the spot"]
      ]
    }
  ];
  ...
  function sourceFidelity(a) {
    ...
    const hub = /teacher hub|hub /i.test(kind) || /teacher hub/i.test(source);
    const guide = /guide|best buddies|program|activity|workbook|calendar/i.test(kind + " " + source);

    if (hub && !guide) return { label: "Teacher Hub planning support", tone: "hub" };
    if (hub && guide) return { label: "Best Buddies activity + Hub option", tone: "mixed" };
    if (/hub/i.test(kind)) return { label: "Best Buddies activity + Hub option", tone: "mixed" };
  ...
  function buildPlan(mode, excludeMainIds = []) {
    ...
    const welcomePool = poolFor(month, "welcome");
    const mainPool = poolFor(month, "main");
    const closePool = poolFor(month, "close");

    const welcome = pickStage(welcomePool, mode);
    const close = pickStage(closePool, mode);
    const base = (Number(acts[welcome]?.minutes) || 5) + (Number(acts[close]?.minutes) || 5);
    ...
    const ranked = rank(candidatePool, state.answers, mode)
      .map((x) => {
        ...
        const timingPenalty = mins > available + 8 ? (mins - available) * 1.5 : Math.abs(available - mins) * 0.08;
    ...
    for (const candidate of ranked) {
      ...
      const allowance = Math.max(5, target >= 45 ? 12 : 7);
    ...
    const communicationRequested = state.answers.focus === "communication" || state.answers.fit.includes("communication");
    ...
    const note = mode === "simple"
      ? "Keeps materials, setup, and room logistics as light as possible."
      : communicationRequested
        ? "Leans into activities where communication can happen through more than speech."
        : "Prioritizes flexible participation, choice, and more than one way to join in.";

    return {
      label,
      note,
      mode,
      ids: [welcome, ...mains, close].filter(Boolean)
    };
  }
  ...
  function currentPresetPlan() {
    ...
    if (!ids.length) {
      ids = [
        shortest(poolFor(monthSelect.value, "welcome")),
        shortest(poolFor(monthSelect.value, "main")),
        shortest(poolFor(monthSelect.value, "close"))
      ].filter(Boolean);
    }
    return {
      label: "Quick 30-minute plan",
      note: "A simple month-based starting point you can use as-is or adjust.",
      mode: "quick",
      ids
    };
  }
  ...
  function stage(id) {
    return acts[id]?.type === "welcome" ? "Welcome" : acts[id]?.type === "close" ? "Wrap-up" : "Main";
  }
  ...
  function renderHome() {
    ...
    root.innerHTML = `<section class="gb-panel">
      <div class="gb-panel-head">
        ...
        <p>Two ways in. Use the guided builder when you want the Hub to do the matching, or choose the pieces yourself.</p>
      </div>

      <div class="gb-paths">
        <button class="gb-path guided" type="button" data-path="guided">
          <small>Five quick choices</small>
          <strong>Build it for me</strong>
          <span>Tell us about your time, group, goal, prep, and what will make participation easier. We’ll build two genuinely different options.</span>
          <b>→</b>
        </button>
        ...
        <button class="gb-path manual" type="button" data-path="manual">
          ...
          <span>Choose a welcome, main activity, and wrap-up. Participation badges and source notes stay visible while you browse.</span>
          <b>→</b>
        </button>
      </div>
  ...
  function renderQuestion() {
    ...
    root.innerHTML = `<section class="gb-panel">
      <div class="gb-panel-head">
        ...
        <p>We’re matching the activity to the gathering, not asking you to label anyone in the room.</p>
      </div>
  ...
  function renderResults() {
    ...
    root.innerHTML = `<section class="gb-panel">
      <div class="gb-panel-head">
        ...
        <h2>Two good ways to run it.</h2>
        <p>These aren’t the same plan with one activity swapped. One keeps logistics especially simple. The other deliberately opens up more ways to participate.</p>
  ...
  function candidateAlternatives(currentId, plan) {
    const type = acts[currentId]?.type || "main";
    const pool = poolFor(monthSelect.value, type);
    const exclude = plan.ids.filter((id) => id !== currentId);
    return rank(pool, state.answers, plan.mode === "simple" ? "simple" : "participation", exclude).map((x) => x.id);
  }
  ...
  function renderFinal(plan) {
    ...
    root.innerHTML = `<section class="gb-panel">
      <div class="gb-panel-head">
        ...
        <h2>Your gathering plan.</h2>
        <p>Everything important is here. Keep the participation details closed when you want a quick glance, or open them when you’re adapting the plan for your group.</p>
      </div>
  ...
  function finalItem(id, index) {
    ...
    return `<article class="gb-final-item">
      ...
      <p>${esc(a.summary || "")}</p>
      ${badgesMarkup(id, 5)}
      ${showReason ? `<div class="gb-fit-reason"><strong>Why this fits:</strong> ${esc(fitReasonText(id))}</div>` : ""}
      ...
      <details class="gb-access-detail">
        <summary>Make this easier to participate in</summary>
  ...
  function renderManual() {
    ...
    const month = monthSelect.value;
    const groups = [
      ["welcome", "1 · Choose a welcome", "Keep the first few minutes easy to enter and easy to pass."],
      ["main", "2 · Choose one main activity", "Add a second only when you genuinely have the time."],
      ["close", "3 · Choose a wrap-up", "Use the last few minutes to reflect, appreciate, or let members shape what comes next."]
    ];
    ...
    root.innerHTML = `<section class="gb-panel">
      <div class="gb-panel-head">
        ...
        <p>The badge row gives you the quick read. Open “Ways to join + source” only when you want the detail.</p>
      </div>

      <div class="gb-manual">
        <div class="gb-manual-intro">
          <p><strong>No separate accessibility track.</strong> Communication, movement, reading/writing, partner support, choice, and source-supported adaptations stay visible inside the same activity bank.</p>
        </div>
        ...
        ${groups.map(([type, title, help]) => `<section class="gb-manual-stage">
          ...
          <div class="gb-activity-grid">
            ${poolFor(month, type).map((id) => activityCard(id)).join("")}
          </div>
        </section>`).join("")}
  ...
  function manualIds() {
    return [...state.manual.welcome, ...state.manual.main, ...state.manual.close];
  }

  function activityCard(id) {
    const a = acts[id];
    if (!a) return "";
    const type = a.type || "main";
  ...
  function toggleManual(id) {
    ...
    } else if (type === "welcome" || type === "close") {
  ...
  monthSelect.addEventListener("change", () => {
    state.generated = [];
    state.chosen = null;
    state.manual = { welcome: [], main: [], close: [] };
    renderHome();
  });
